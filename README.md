...
- **File attachments on Notes** — attach any file type (Word/Excel/PowerPoint docs, PDFs, images, zips, anything), up to 50MB each. Stored locally in your browser (IndexedDB) — works in both the installed copy and the hosted version, no restrictions on file type. Export/Import carries the attachment *list* (filenames) along with a note, but not the file content itself — the actual files live only in the browser that uploaded them, so re-attach on a new browser/device if needed.
